...
\begin{document}
	...
	\begin{frame}
		...
		\begin{frame}
				\frametitle{Notions et règles de franchissement }
	\textit{\textbf{Exemple}}
	     Soit P = $p_{1}$ , $p_{2}$ , $p_{3}$ où $p_{1}$ représente la ressource $«$fichier$»$, $p_{2}$ la ressource $«$imprimante$»$ et p 3 la ressource «fichiers en cours d’impression».
...
\textbf{	 C = 	$C^{+}$ - $C^{-}$}
	...
	$M_{1}$ est tirable car [4, 1, 1] $\geq$ [1, 1, 0] c.-à-d $M_{1}$ $\geq$ $t_{1}$ $C^{-}$
...
${}^tC$ =	
	...
	\begin{examplesecond}
		...
		Soit $T^{*}$ l’ensemble de transitions $T^{\ast}$ = t1 , t2 , . . . , tn\\
	...
	\begin{minipage}[h]{0.6\linewidth}
		...
		Alors le vecteur d'occurrences $\overrightarrow{\sigma} = (\overrightarrow{\sigma} ($t_{1}$ ), \overrightarrow{\sigma} ($t_{2}$ ), \overrightarrow{\sigma} ($t_{3}$ )) est \overrightarrow{\sigma}$\\
		= (2, 1, 0)\\
	\end{minipage}
	...
	\begin{center}
	$\forall M \inA ,\sum_{i=1}^{size(P)} M_{0}(p_{i}) = \sum_{i=1}^{size(P)} M(p_{i})$
	...
	\item  2-borné.
	\item  non-vivant (non peusdo-vivant : $\forall M, \exists t/M [t \geq)$
	\item  quasi-vivant ($ \forall t, \exists M/M [t \geq $)
